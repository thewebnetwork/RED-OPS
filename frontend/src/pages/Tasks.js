...
import {
  ...
  MoreHorizontal, Calendar, Tag, User, Loader2, Grip, Trash2,
...
const STATUS_RMAP = { backlog: 'Backlog', todo: 'Todo', doing: 'In Progress', waiting_on_client: 'In Progress', review: 'In Review', done: 'Done' };
...
function TaskDetail({ task, onClose, onRefresh, onDelete }) {
  ...
  const handleStatusChange = async (newStatus) => {
    try {
      await ax().patch(`${API}/tasks/${task.id}`, { status: STATUS_MAP[newStatus] || newStatus });
      onRefresh();
    } catch { toast.error('Failed to update status'); }
  };

  return (
    <div style={{ position: 'fixed', top: 0, right: 0, bottom: 0, width: 420, background: 'var(--bg-card)', borderLeft: '1px solid var(--border)', zIndex: 100, display: 'flex', flexDirection: 'column', animation: 'slideRight 0.15s ease both', boxShadow: '-4px 0 20px rgba(0,0,0,0.15)' }}>
      {/* Header */}
      <div style={{ padding: '16px 18px', borderBottom: '1px solid var(--border)', flexShrink: 0 }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 8 }}>
          <span style={{ fontSize: 10, fontWeight: 600, color: 'var(--tx-3)', textTransform: 'uppercase', letterSpacing: '0.05em' }}>Task Detail</span>
        ...
        <h3 style={{ fontSize: 15, fontWeight: 700, margin: 0, color: 'var(--tx-1)' }}>{task.title}</h3>
        {task.description && (
          <p style={{ fontSize: 12.5, color: 'var(--tx-2)', margin: '8px 0 0', lineHeight: 1.5 }}>{task.description}</p>
        )}
      </div>
      ...
      {subtasks.length > 0 && (
        <div style={{ padding: '12px 18px', borderBottom: '1px solid var(--border)' }}>
          <div style={{ fontSize: 11, fontWeight: 600, color: 'var(--tx-3)', textTransform: 'uppercase', letterSpacing: '0.05em', marginBottom: 8 }}>
          ...
          {subtasks.map(st => (
            <div key={st.id} style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '4px 0' }}>
              {st.status === 'done' ? <CheckCircle2 size={13} color="#22c55e" /> : <Circle size={13} color="var(--tx-3)" />}
              <span style={{ fontSize: 12, color: st.status === 'done' ? 'var(--tx-3)' : 'var(--tx-1)', textDecoration: st.status === 'done' ? 'line-through' : 'none' }}>{st.title}</span>
            </div>
          ))}
        </div>
      )}
      ...
      <div style={{ flex: 1, overflowY: 'auto', padding: '12px 18px' }}>
        <div style={{ fontSize: 11, fontWeight: 600, color: 'var(--tx-3)', textTransform: 'uppercase', letterSpacing: '0.05em', marginBottom: 10, display: 'flex', alignItems: 'center', gap: 6 }}>
...
export default function Tasks() {
  ...
  const fetchTasks = useCallback(async () => {
    try {
      ...
      if (err.response?.status !== 401) console.error('Failed to load tasks');
    }
    setLoading(false);
  }, []);
