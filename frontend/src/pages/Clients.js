  ...
  return (
    ...
    >
      ...
      <div style={{ display: 'flex', alignItems: 'flex-start', gap: 10, marginBottom: 12 }}>
        <div style={{ width: 40, height: 40, borderRadius: '50%', background: 'var(--red)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 16, fontWeight: 800, color: 'white', flexShrink: 0 }}>
          {(client.name || '?').charAt(0).toUpperCase()}
        </div>
        <div style={{ flex: 1, minWidth: 0 }}>
          <div style={{ fontSize: 14, fontWeight: 700, color: 'var(--tx-1)', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{client.name}</div>
          <div style={{ fontSize: 11.5, color: 'var(--tx-3)', marginTop: 1 }}>{client.email}</div>
        </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 8, marginBottom: 12 }}>
        {[
          ...
          <div key={label} style={{ padding: '7px 8px', background: 'var(--bg-elevated)', borderRadius: 7, textAlign: 'center' }}>
            <div style={{ fontSize: 13, fontWeight: 700, color: 'var(--tx-1)' }}>{value}</div>
      ...
      {client.renewal_date && (
        <div style={{ marginTop: 10, fontSize: 11, color: 'var(--tx-3)', display: 'flex', alignItems: 'center', gap: 4 }}>
          <Clock size={10} />
  ...
  return (
    <div className="page-content" style={{ animation: 'fadeInUp .3s ease' }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 24, flexWrap: 'wrap' }}>
        <div style={{ flex: 1 }}>
          <h1 style={{ margin: 0, fontSize: 22, fontWeight: 800, color: 'var(--tx-1)', letterSpacing: '-.02em' }}>Clients</h1>
          <p style={{ margin: '4px 0 0', fontSize: 12.5, color: 'var(--tx-3)' }}>{clients.length} accounts · health-scored</p>
        </div>
        ...
        <button onClick={load} style={{ display: 'flex', alignItems: 'center', gap: 5, padding: '7px 12px', background: 'var(--bg-elevated)', border: '1px solid var(--border)', borderRadius: 7, cursor: 'pointer', color: 'var(--tx-3)', fontSize: 12 }}>
      ...
      <div style={{ display: 'flex', gap: 8, marginBottom: 20, flexWrap: 'wrap' }}>
        {[
          ...
          <button
            ...
            style={{
              padding: '6px 14px', borderRadius: 7, border: `1px solid ${filter === key ? color : 'var(--border)'}`,
              background: filter === key ? color + '15' : 'transparent',
              cursor: 'pointer', fontSize: 12, fontWeight: filter === key ? 700 : 500,
              color: filter === key ? color : 'var(--tx-3)', transition: 'all .15s',
            }}
          >{label}</button>
        ))}
      </div>
      ...
      {loading ? (
        ...
        <div style={{ textAlign: 'center', padding: '60px 0', color: 'var(--tx-3)' }}>
          ...
          <div style={{ fontSize: 15, fontWeight: 600, color: 'var(--tx-2)' }}>
      ...
      {/* Client Detail Panel */}
      {selected && (
        <div style={{ position: 'fixed', right: 0, top: 0, bottom: 0, width: 380, background: 'var(--bg-card)', borderLeft: '1px solid var(--border)', zIndex: 200, display: 'flex', flexDirection: 'column', animation: 'slideInRight .2s ease' }}>
          <div style={{ padding: '16px', borderBottom: '1px solid var(--border)', display: 'flex', alignItems: 'center', gap: 10 }}>
            <div style={{ width: 40, height: 40, borderRadius: '50%', background: 'var(--red)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 16, fontWeight: 800, color: 'white' }}>
              {(selected.name || '?').charAt(0).toUpperCase()}
            </div>
            <div style={{ flex: 1 }}>
              <div style={{ fontSize: 15, fontWeight: 700, color: 'var(--tx-1)' }}>{selected.name}</div>
              <div style={{ fontSize: 11.5, color: 'var(--tx-3)' }}>{selected.email}</div>
            </div>
            <button onClick={() => setSelected(null)} style={{ background: 'none', border: 'none', cursor: 'pointer', color: 'var(--tx-3)' }}><X size={16} /></button>
          </div>
          <div style={{ flex: 1, overflowY: 'auto', padding: '16px' }}>
            ...
            <div style={{ marginTop: 16, display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10 }}>
              {[
                ...
                <div key={label} style={{ padding: '10px', background: 'var(--bg-elevated)', borderRadius: 8 }}>
                  <div style={{ fontSize: 10, color: 'var(--tx-3)', textTransform: 'uppercase', letterSpacing: '.05em', marginBottom: 3 }}>{label}</div>
                  <div style={{ fontSize: 13, fontWeight: 600, color: 'var(--tx-1)' }}>{value}</div>
                </div>
              ))}
            </div>
            {/* Recent orders */}
            <div style={{ marginTop: 16 }}>
              <div style={{ fontSize: 11, fontWeight: 700, color: 'var(--tx-3)', letterSpacing: '.07em', textTransform: 'uppercase', marginBottom: 8 }}>Recent Requests</div>
              {orders.filter(o => o.requester_id === selected.id).slice(0, 5).map(o => (
                <div key={o.id} style={{ padding: '8px 10px', background: 'var(--bg-elevated)', borderRadius: 7, marginBottom: 5, fontSize: 12 }}>
                  <div style={{ fontWeight: 500, color: 'var(--tx-1)', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{o.title || o.request_type || 'Request'}</div>
                  <div style={{ fontSize: 11, color: 'var(--tx-3)', marginTop: 2 }}>{o.status} · {o.order_code}</div>
                </div>
              ))}
              {orders.filter(o => o.requester_id === selected.id).length === 0 && (
                <div style={{ fontSize: 12, color: 'var(--tx-3)', padding: '10px', textAlign: 'center' }}>No requests yet</div>
              )}
            </div>
          </div>
        </div>
