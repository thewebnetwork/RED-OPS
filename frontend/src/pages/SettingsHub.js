...
function SettingsItem({ item, onClick }) {
  ...
  return (
    <button
      ...
      className="w-full flex items-center gap-4 p-4 rounded-xl border bg-white hover:border-[#A2182C]/30 hover:shadow-md transition-all group text-left"
    >
      ...
      <div className="flex-1 min-w-0">
        <p className="font-semibold text-sm group-hover:text-[#A2182C] transition-colors">
          {item.name}
        </p>
        <p className="text-xs mt-0.5 leading-relaxed">{item.description}</p>
      </div>
      <ChevronRight
        size={16}
            ...
            {currentSection && (
              <>
                <div className="mb-4 flex items-center gap-2">
                  <currentSection.icon size={16} className="" strokeWidth={1.75} />
